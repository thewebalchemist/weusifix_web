...
const ServicesPage: React.FC = () => {
  ...
  const fetchListings = async () => {
    try {
      ...
      if (selectedCategory) {
        query = query.eq('serviceCategory', selectedCategory);
      }
      if (providerType) {
        query = query.eq('isIndividual', providerType === 'individual');
      }
    ...
    <Link href={`/services/${listing.slug}`} key={listing.id}>
      <div className="relative rounded-2xl bg-white dark:bg-gray-800 cursor-pointer shadow-md">
        ...
        <div className="p-4">
          ...
          <h2 className='text-md font-semibold text-gray-800 dark:text-white mb-1'>{listing.title.substring(0, 25)}...</h2>
